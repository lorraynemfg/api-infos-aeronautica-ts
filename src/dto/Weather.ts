...
export function translateWeather(metarObj: { mens: string }): WeatherData {
    const { mens } = metarObj;
    ...
    function translateWindDirection(degrees: number) {
      const cardinalDirections = ['Norte', 'Nordeste', 'Leste', 'Sudeste', 'Sul', 'Sudoeste', 'Oeste', 'Noroeste'];
      const index = Math.round(degrees / 45) % 8;
      return cardinalDirections[index];
    }
  
    function translateCloudConditions(cloudConditions: string) {
